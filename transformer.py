...
class TransformerEncoder(object):
    ...
    def encode(self, source_ids, source_mask):
        ...
        def _prepare_source():
            ...
            # ########################################### PRINT #########################################################
            # saver = tf.compat.v1.train.Saver({"embedding_table": self.embedding_layer.embedding_table})
            # init_op = tf.compat.v1.initialize_all_variables()
            printops = []
            # printops.append(tf.compat.v1.Print([], [self.embedding_layer.embedding_table], "embedding_table", summarize=10000))
            ...
            # tf.io.write_file("/cs/labs/gabis/bareluz/nematus/output_translate.txt",printops)
            # with tf.compat.v1.Session() as sess:
            #     sess.run(init_op)
            #     saver.save(sess, "/cs/labs/gabis/bareluz/nematus/output_translate.ckpt")
            with tf.control_dependencies(printops):
                # with open("/cs/labs/gabis/bareluz/nematus/output_translate.txt", 'wb') as file_:
                #     for i in range(29344):
                #         embed = self.embedding_layer.embedding_table[i, :]
                #         # word = word_to_idx[i]
                #         np_embedding = tf.make_ndarray(embed.op.get_attr('value'))
                #         pickle.dump(np_embedding,file_)
