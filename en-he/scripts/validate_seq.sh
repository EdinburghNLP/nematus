...
#data_dir=/cs/snapless/oabend/borgr/SSMT/preprocess/data/en_he/20.06.07/UD
...
if [ ! -f $ref ] ; then
	...
	if [ ! -f $trns ] ; then
		$script_dir/postprocess.sh < "$data_dir/$dev_prefix.unesc.tok$trg_tc.bpe.trns1.$trg" > "$trns"
	fi
