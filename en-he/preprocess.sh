...
#echo "coppied ${SCRIPT_DIR}/${SCRIPT_FILE} to ${workdir}/${SCRIPT_FILE}"
...
#python $subword_nmt/get_vocab.py < $workdir/anti.unesc.tok.tc.bpe.$SRC > $workdir/vocab.clean.unesc.tok.tc.bpe.$SRC
#python $subword_nmt/get_vocab.py < $workdir/anti.unesc.tok.bpe.$TRG > $workdir/vocab.clean.unesc.tok.bpe.$TRG
...
#
#
if [ -f "$workdir/anti.clean.unesc.tok.bpe.$SRC$TRG" ]; then
   rm ${workdir}/tmp* -rf
