...
def build_model(tparams, options):
    ...
    Wemb_orig = None
    if options['use_dropout']:
        if options['map_dropout']:
            Wemb_orig = theano.shared(tparams['Wemb'].get_value(), name= "Wemb_orig")
            emb_orig = Wemb_orig[x.flatten()].reshape([n_timesteps, n_samples, options['dim_word']])
            emb = emb * source_dropout + emb_orig * (1-source_dropout)
        else:
            emb *= source_dropout
    ...
    if options['use_dropout']:
        if options['map_dropout']:
            embr_orig = Wemb_orig[xr.flatten()].reshape([n_timesteps, n_samples, options['dim_word']])
            emb = embr * source_dropout[::-1] + embr_orig * (1-source_dropout[::-1])
        else:
    ...
    if options['use_dropout']:
        if options['map_dropout']:
            Wemb_dec_orig = theano.shared(tparams['Wemb_dec'].get_value(), name= "Wemb_dec_orig")
            emb_dec_orig = Wemb_dec_orig[y.flatten()].reshape([n_timesteps_trg, n_samples, options['dim_word']])
            emb_shifted_orig = tensor.zeros_like(emb_dec_orig)
            emb_shifted_orig = tensor.set_subtensor(emb_shifted_orig[1:], emb_dec_orig[:-1])
            emb_dec_orig = emb_shifted_orig
            emb = emb * target_dropout + emb_dec_orig * (1-target_dropout)
        else:
            emb *= target_dropout
