    ...
    def __init__(self, config, context, x_mask,
        ...
        with tf.name_scope("base"):
            ...
            self.att_grustep2 = DeepTransitionRNNWithMultiHopAttentionStep(
                                    ...
                                    attention_step_class = DotProductAttentionStep if config.dec_attention_dot_product else AttentionStep,
                                    attention_step_options={'hidden_size' : config.dec_attention_hidden_dim if config.dec_attention_hidden_dim != -1 else 2*config.state_size,
                                                            'use_layer_norm' : config.use_layer_norm,
                                                            'dropout_context' : dropout_hidden,
                                                            'dropout_state' : dropout_hidden,
                                                            'projection_dim': config.dec_attention_projection_dim,
                                                            'n_attention_heads': config.dec_attention_heads},
