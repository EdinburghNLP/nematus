...
def create_model(config, sess, ensemble_scope=None, train=False):
    ...
    elif config.reload is not None:
        reload_filename = config.reload
    if (reload_filename is None) and (config.prior_model is not None):
...
def train(config, sess):
    assert config.prior_model != None and (tf.train.checkpoint_exists(os.path.abspath(config.prior_model))) or (
                config.map_decay_c == 0.0), \
    ...
    if config.summaryFreq:
        summary_dir = config.summary_dir if (config.summary_dir != None) else (
            os.path.abspath(os.path.dirname(config.saveto)))
    ...
    for progress.eidx in range(progress.eidx, config.max_epochs):
        ...
        for source_sents, target_sents in text_iterator:
            ...
            if x_in is None:
                ...
                continue
            write_summary_for_this_batch = config.summaryFreq and ((progress.uidx % config.summaryFreq == 0) or (
            ...
            if config.dispFreq and progress.uidx % config.dispFreq == 0:
                ...
                logging.info(
                    '{} Epoch: {} Update: {} Loss/sentence: {} Loss/word: {} Words/sec: {} Sents/sec: {}'.format(
                        disp_time, progress.eidx, progress.uidx, total_loss / n_sents, total_loss / n_words,
    ...
    def translate_worker(in_queue, out_queue, model, sess, config):
        while True:
            ...
            if job is None:
                break
            idx, x = job
            y_dummy = np.zeros(shape=(len(x), 1))
...
def validate(config, sess, valid_text_iterator, model, normalization_alpha=0):
    ...
    logging.info(
        'Validation loss (AVG/SUM/N_SENT): {0} {1} {2}'.format(total_loss / total_seen, total_loss, total_seen))
    return costs
...
def parse_args():
    ...
    if config.source_vocab_sizes == None:
        ...
        logging.error(
            'too many values supplied to \'--source_vocab_sizes\' option (expected one per factor = {0})'.format(
                config.factors))
        sys.exit(1)
