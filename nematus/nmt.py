...
def build_model(tparams, options):
    ...
    if options['decoder'] == 'gru_cond_sa':
        # 'gru_cond_sa decoder requires 3D init_state
        init_state = tensor.unbroadcast(tensor.shape_padleft(init_state), 0)
    ...
    if options['decoder'] == 'gru_cond_sa':
        # 'gru_cond_sa decoder produces a list of 3D tensors (state histories)
        state_histories = proj[0]
        last_history = state_histories[-1]
        except_initial_state = last_history[1:]
        proj_h = except_initial_state
    else:
        proj_h = proj[0]
...
def build_sampler(tparams, options, use_noise, trng, return_alignment=False):
    ...
    if options['decoder'] == 'gru_cond_sa':
        # 'gru_cond_sa decoder requires 3D init_state
        init_state = tensor.shape_padleft(init_state)
    ...
    if options['decoder'] == 'gru_cond_sa':
        state_history = proj[0]
        next_state = state_history[-1]
    else:
        next_state = proj[0]
    ...
    if options['decoder'] == 'gru_cond_sa':
        outs = [next_probs, next_sample, state_history]
    else:
...
def gen_sample(f_init, f_next, x, trng=None, k=1, maxlen=30,
    ...
    self_attention = next_state[0].ndim == 3 # TODO: this assumes each model uses self_attention

    # x is a sequence of word ids followed by 0, eos id
    for ii in xrange(maxlen):
        for i in xrange(num_models):
            ctx = numpy.tile(ctx0[i], [live_k, 1]) # ctx.shape = (src_len, batch*live_k, dim*1)
        ...
        if stochastic:
            ...
            probs = sum(next_p)/num_models # average all the matrices to get single (batch, vocab_size) 
            ...
            voc_size = next_p[0].shape[1] # next_p is a list of matrices (batch, vocab_size)
            ...
            for idx, [ti, wi] in enumerate(zip(trans_indices, word_indices)):
                ...
                if self_attention:
                    new_hyp_states.append([copy.copy(next_state[i][:,ti]) for i in xrange(num_models)]) # for every model, take the ti-th state
                else:
                    new_hyp_states.append([copy.copy(next_state[i][ti]) for i in xrange(num_models)]) # for every model, take the ti-th state
                if return_alignment:
            ...
            if self_attention:
                next_state = [numpy.stack(state,axis=1) for state in zip(*hyp_states)]
            else:
