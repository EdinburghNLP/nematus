...
def sample(session, model, x, x_mask, target_lang, graph=None):
    ...
    feed_dict[model.inputs.target_lang_id] = target_lang
    if graph is None:
...
def beam_search(session, models, x, x_mask, beam_size, target_lang, graph=None):
...
def construct_sampling_ops(model):
    ...
    def body(i, prev_base_state, prev_high_states, prev_y, prev_emb,
        ...
        new_emb = decoder.y_emb_layer.forward(new_y,
                                              model.inputs.target_lang_id)
        ...
        # slice).
        new_emb.set_shape([None, decoder.embedding_size])
